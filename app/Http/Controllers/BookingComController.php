...
class BookingComController extends Controller
{
    ...
    public function webhook(Request $request)
    {
        ...
        try {
            ...
            $checkIn = Carbon::parse($data['checkin'] ?? now());
            $checkOut = Carbon::parse($data['checkout'] ?? now()->addDay());
    ...
    public function pullReservations(Request $request)
    {
        ...
        try {
            ...
            foreach ($reservations as $res) {
                ...
                if (Reservation::where('ota_booking_id', $otaId)->exists()) {
                    continue;
                }

                $channel = Channel::where('ota_name', 'booking_com')->first();
                $room = Room::where('property_id', $channel?->property_id)->first();
                $checkIn = Carbon::parse($res['checkin']);
                $checkOut = Carbon::parse($res['checkout']);
                $nights = $checkIn->diffInDays($checkOut);
    ...
    public function pushRates(Request $request)
    {
        ...
        if ($errors > 0) {
            $message .= " {$errors} failed — check logs.";
        }
